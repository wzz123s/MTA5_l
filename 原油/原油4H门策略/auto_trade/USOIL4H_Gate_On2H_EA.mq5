//+------------------------------------------------------------------+
//|                  USOIL4H_Gate_On2H_EA.mq5                       |
//|                                                     Codex       |
//|  4H 机会门 -> 2H 交易（对齐 validate_usoil_gate_strategy.py 4H门）|
...
input group "=== Account ==="
input ulong   InpMagic      = 362137;      // Magic Number
...
input bool   InpEventFilterOn   = true;   // 事件过滤总开关（信号级复核 T=4h：收益持平+PF 1.73->2.18+MaxDD-21%）
input int    InpEventFilterHrs  = 4;  // 高影响事件前 N 小时内不开新仓（复核修正：8h 会误伤边界赢单）
input int    InpEventCheckEvery = 300;    // 日历查询缓存（秒）


//--- globals
string g_signals_csv  = "USOIL4H_gate_signals_export.csv";
string g_ledger_csv   = "USOIL4H_gate_trade_ledger.csv";
