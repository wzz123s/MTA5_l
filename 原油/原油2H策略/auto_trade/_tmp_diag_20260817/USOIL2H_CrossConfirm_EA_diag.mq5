...
input group "=== Runtime ==="
input int     InpHistoryBars = 600;        // H2 bars to load
...
void CsvLedgerHeader()
{
   ...
   if(h != INVALID_HANDLE)
   {
      FileWrite(h, "signal_time", "entry_time", "dir", "entry", "stop", "stop_pct",
                "exit_time", "exit_price", "reason", "pnl_points", "pnl_usd", "virtual_balance");
      FileClose(h);
   }
}
...
void ProcessCandidates(datetime bar0_time, int n, const double &way[], const double &vol_way[])
{
   for(int c = ArraySize(g_cands) - 1; c >= 0; c--)
   {
      ...
      if(g_cands[c].age == 2 && n >= 4)
      {
         ...
         // BUG-1 修复(2026-09-04): way_s_way/vol_way_s_way 无符号 [0,1], 不能乘 sign(否则空单恒不通过)
...
void ProcessOpenTrades(const MqlRates &rates[], int n, const int &raw[])
{
   for(int s = 0; s < ArraySize(g_trades); s++)
   {
      ...
      // FIX: always check the most recent closed bar (rates[n-2]).
      // Old formula (n-2)-(age-1) walked BACKWARD as age grew, so trades
      // were checked against stale bars and never exited.
      int idx = n - 2;
      if(idx < 0) continue;
...
void OnTick()
{
   ...
   MqlRates rates[];
   int n = CopyRates(InpSymbol, PERIOD_H2, 0, InpHistoryBars, rates);
   if(n < 80) return;

   // --- DIAG: verify CopyRates array direction (temporary) ---
   if(g_bar_seq <= 8 || g_bar_seq % 200 == 0)
   {
      Print("[DIAG] seq=", g_bar_seq, " bar0=", TimeToString(bar0_time),
            " n=", n,
            " r0=", TimeToString(rates[0].time),
            " r1=", TimeToString(rates[1].time),
            " r2=", TimeToString(rates[2].time),
            " rN2=", TimeToString(rates[n-2].time),
            " rN1=", TimeToString(rates[n-1].time));
   }
