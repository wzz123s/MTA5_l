...
- 2026-09-09 **版本记录机制落地（本地 + GitHub 远程备份接入）**：按用户裁决——EA 源码 `#property version` 为机器可读发布版本、仅在发布/认证基线通过时递增，发布按 `策略名-EA名-vX.Y` 打 tag；8 个策略新增 `版本记录.md`（现役状态／发布基线表／待发布变更），`00_项目规则.md` R1/R9 写入落地条款与版本载体约定；git 身份改用 wzz／376278901@qq.com；origin = https://github.com/wzz123s/MTA5_l，已首推 master 9 提交。版本记录样板见 `黄金\30m2H策略\版本记录.md`
