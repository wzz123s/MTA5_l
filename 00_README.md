...
> **未动（等拍板）**：T11 / T13 / T15 / T16 / T21 —— 其中 T11（2H ABC 与 30m2H ABC 补 `SetExpertMagicNumber`）、T13（拆双挂）、T15（MCT bar 序列）属 A/D 项：需重编译 + 重挂图表，会短暂打断 EA（AGENTS.md C 段禁止擅自打断），且 `auto_trade\` 属冻结区。用户已定顺序：**T20（已完成）→ 排 A/D 打断窗口 → G1 并入 T1**。
