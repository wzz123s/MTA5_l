...
> **未动（等拍板）**：T11 / T13 / T15 / T16 / T20 / T21 —— 其中 T11（2H ABC 与 30m2H ABC 补 `SetExpertMagicNumber`）、T13（拆双挂）、T15（MCT bar 序列）属 A/D 项：需重编译 + 重挂图表，会短暂打断 EA（AGENTS.md C 段禁止擅自打断），且 `auto_trade\` 属冻结区；T20（三个部署脚本 chart 编号失效）修好前**不要重跑那三个脚本**。
