...
| T11 | **P0 归因 BUG｜🔧 代码已修 + 编译 0 errors，待部署**（09-09 03:10）：`2H_M30_6H_ABC_EA` / `30m2H_ABC_EA` 已补 `g_trade.SetExpertMagicNumber(InpMagic)`（照抄 1H ABC L719-723 已验证范式，注释内写明实测证据）；原缺陷致平仓 deal 落 magic=0、**15/59 持仓开平错配**，按 magic 统计使 2H ABC 显示 −279.62（真实 **+68.17**）、1H ABC −68.82（真实 **+161.76**）、主线 S3 0.00（真实 **+177.87**）。**A3（主线 stage 级平仓 magic）按裁决暂缓**：血缘归因已覆盖其后果，且主线 v3.37 正在 T1 验收中 | 问题记录 §二十三③④⑤ + A/D 阶段一 |
