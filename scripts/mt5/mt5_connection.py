...
# Main (monitoring) MT5 terminal install. All monitor/data connections must target
# this install explicitly: if a second "debug" terminal (e.g. MetaTrader 5 EXNESS)
# is started last, the package's default-instance connection would otherwise drift
# to it, and the monitor would read the wrong terminal's positions/history.
...
def initialize_mt5(
    *,
    ...
    if not terminal_path:
        # Lock to the main monitoring terminal unless a caller overrides.
        terminal_path = MAIN_TERMINAL_PATH
    if terminal_path:
