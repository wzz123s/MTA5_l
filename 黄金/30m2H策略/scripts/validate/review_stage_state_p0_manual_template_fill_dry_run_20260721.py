...
DRY_RUN_DIR = VALIDATION_DIR / "stage_state_p0_manual_template_fill_dry_run_20260721"
OUT_DIR = VALIDATION_DIR / "stage_state_p0_manual_template_fill_dry_run_review_20260721"

PREREQ_PLAN = PREREQ_DIR / "manual_evidence_prerequisite_plan.csv"
MANIFEST = DRY_RUN_DIR / "p0_manual_template_fill_dry_run_manifest.csv"
DECISION = DRY_RUN_DIR / "p0_manual_template_fill_dry_run_decision.json"
...
def json_fields_valid(path: Path, fields: List[str]) -> bool:
    ...
    for field in fields:
        ...
        if not (value.startswith("PENDING_") or value.startswith("REDACTED_")):
            return False
    return True


def markdown_fields_valid(path: Path, fields: List[str]) -> bool:
    text = path.read_text(encoding="utf-8-sig")
    for field in fields:
        pattern = rf"`{re.escape(field)}`:\s*`(PENDING|REDACTED)_"
...
def main() -> int:
    ...
    for row in manifest_rows:
        path = resolve_workspace_path(row.get("dry_run_relative_path", ""))
        try:
        ...
        if path.suffix.lower() == ".json":
            valid = json_fields_valid(path, fields)
        else:
            valid = markdown_fields_valid(path, fields)
        if not valid:
            field_failures.append(f"{row.get('gap_id')}:{row.get('file_name')}")

    all_text = output_text()
    checks = [
        check("decision_status_live_blocked", decision.get("status") == "p0_manual_dry_run_generated_live_blocked", str(decision.get("status"))),
        check("expected_p0_manual_count_is_10", len(expected) == 10, f"expected={len(expected)}"),
        ...
        check("no_runner_mt5_live_set_actions", decision.get("runner_executed") is False and decision.get("mt5_accessed") is False and decision.get("live_set_switched") is False, "action flags"),
    ]
    failures = [row for row in checks if not row["passed"]]
    review_decision = {
        "decision_time": reviewed_at,
        "check_id": "stage_state_p0_manual_template_fill_dry_run_review",
        "status": "pass_p0_manual_dry_run_live_blocked" if not failures else "fail_p0_manual_dry_run_review",
        "review_completed": True,
        "p0_manual_items": len(expected),
        "dry_run_files_reviewed": len(manifest_rows),
        "markdown_files": sum(1 for row in manifest_rows if row.get("file_kind") == "markdown"),
        ...
        "recommended_next_action": "draft_p0_rehearsal_prerequisite_dry_run" if not failures else "fix_p0_manual_dry_run",
    }

    write_csv(OUT_DIR / "p0_manual_template_fill_dry_run_review_checks.csv", checks, ["check_id", "passed", "severity", "detail"])
    write_csv(OUT_DIR / "p0_manual_template_fill_dry_run_review_decision.csv", [review_decision], list(review_decision.keys()))
    write_json(OUT_DIR / "p0_manual_template_fill_dry_run_review_decision.json", review_decision)
    lines = [
        ...
        f"- p0_manual_items: `{review_decision['p0_manual_items']}`",
    ...
    lines.append("")
    (OUT_DIR / "p0_manual_template_fill_dry_run_review.md").write_text("\n".join(lines), encoding="utf-8-sig")
