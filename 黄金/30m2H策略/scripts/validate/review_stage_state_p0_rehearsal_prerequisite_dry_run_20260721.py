...
PLAN_DIR = VALIDATION_DIR / "stage_state_p0_rehearsal_prerequisite_dry_run_20260721"
OUT_DIR = VALIDATION_DIR / "stage_state_p0_rehearsal_prerequisite_dry_run_review_20260721"

PREREQ_PLAN = PREREQ_DIR / "manual_evidence_prerequisite_plan.csv"
PLAN = PLAN_DIR / "p0_rehearsal_prerequisite_dry_run_plan.csv"
STEPS = PLAN_DIR / "p0_rehearsal_prerequisite_dry_run_steps.csv"
DECISION = PLAN_DIR / "p0_rehearsal_prerequisite_dry_run_decision.json"
...
def main() -> int:
    ...
    for row in plan_rows:
        ...
        if not path.exists():
            ...
            continue
        sample_rows = read_rows(path)
        if not sample_rows or sample_rows[0].get("status") != "NOT_EXECUTED_DRY_RUN_PLAN_ONLY":
    ...
    checks = [
        check("decision_status_live_blocked", decision.get("status") == "p0_rehearsal_dry_run_planned_live_blocked", str(decision.get("status"))),
        check("expected_p0_rehearsal_count_is_1", len(expected) == 1, f"expected={len(expected)}"),
    ...
    review_decision = {
        ...
        "check_id": "stage_state_p0_rehearsal_prerequisite_dry_run_review",
        "status": "pass_p0_rehearsal_dry_run_live_blocked" if not failures else "fail_p0_rehearsal_dry_run_review",
        ...
        "recommended_next_action": "start_p1_manual_template_fill_dry_run_with_redacted_values" if not failures else "fix_p0_rehearsal_dry_run",
    }

    write_csv(OUT_DIR / "p0_rehearsal_prerequisite_dry_run_review_checks.csv", checks, ["check_id", "passed", "severity", "detail"])
    write_csv(OUT_DIR / "p0_rehearsal_prerequisite_dry_run_review_decision.csv", [review_decision], list(review_decision.keys()))
    write_json(OUT_DIR / "p0_rehearsal_prerequisite_dry_run_review_decision.json", review_decision)
    lines = [
        ...
        f"- p0_rehearsal_items: `{review_decision['p0_rehearsal_items']}`",
    ...
    lines.append("")
    (OUT_DIR / "p0_rehearsal_prerequisite_dry_run_review.md").write_text("\n".join(lines), encoding="utf-8-sig")
