...
OUT_DIR = VALIDATION_DIR / "stage_state_p1_manual_template_fill_dry_run_20260721"
...
def p1_manual_rows(prereq_rows: List[Dict[str, str]]) -> List[Dict[str, str]]:
    return [
        row
...
def write_dry_run_file(row: Dict[str, str], index_row: Dict[str, str], fields: List[str], generated_at: str) -> Tuple[Path, str]:
    ...
    if file_kind == "csv" or row["file_name"].endswith(".csv"):
        secret_risk = boolish(row.get("contains_secret_risk"))
        write_csv(out_path, [{field: placeholder(field, secret_risk) for field in fields}], fields)
        return out_path, "csv"
    ...
    raise ValueError(f"Unsupported P1 manual dry-run file type: {row['file_name']}")


def build_report(decision: Dict[str, object], manifest_rows: List[Dict[str, object]]) -> str:
    lines = [
        "# P1 Manual Template Fill Dry Run",
        "",
        "## Decision",
        "",
        f"- status: `{decision['status']}`",
        f"- p1_manual_items: `{decision['p1_manual_items']}`",
        ...
        "- Account/spec data is represented with REDACTED placeholders only.",
        "- No real credentials, MT5 query, runner execution, order action, or live set switch is performed.",
        "",
        "## Files",
        "",
    ]
...
def main() -> int:
    ...
    target_rows = p1_manual_rows(prereq_rows)
    ...
    decision = {
        ...
        "gate_status": "open",
        "ready_to_live_trade": False,
        "recommended_next_action": "review_p1_manual_template_fill_dry_run",
    }
    ...
    write_csv(OUT_DIR / "p1_manual_template_fill_dry_run_manifest.csv", manifest_rows, fieldnames)
    write_csv(OUT_DIR / "p1_manual_template_fill_dry_run_decision.csv", [decision], list(decision.keys()))
    write_json(OUT_DIR / "p1_manual_template_fill_dry_run_decision.json", decision)
    (OUT_DIR / "p1_manual_template_fill_dry_run.md").write_text(build_report(decision, manifest_rows), encoding="utf-8-sig")
