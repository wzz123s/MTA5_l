...
```python
if d == 'up':
    if high[i] > c_up_hp:           # 价格创新高 → 更新价格极值
        c_up_hp  = high[i]
    if sma13[i] > c_up_hs:          # SMA13 创新高 → 独立更新 SMA13 极值
        c_up_hs  = sma13[i]         # (SMA13 是慢速均线，最高值通常晚于价格高点)
        c_up_hw  = wsw[i]
        c_up_hvw = vwsw[i]
...
```python
if d == 'down':
    if low[i] < c_dn_lp:            # 价格创新低 → 更新价格极值
        c_dn_lp  = low[i]
    if sma13[i] < c_dn_ls:          # SMA13 创新低 → 独立更新 SMA13 极值
        c_dn_ls  = sma13[i]         # (SMA13 是慢速均线，最低值通常晚于价格低点)
        c_dn_lw  = wsw[i]
        c_dn_lvw = vwsw[i]
...
| 列名 | 出现行 | 取值 |
|------|--------|------|
| `prev_seg_low_price` | good | 上一 down 段最低价 |
| `prev_seg_low_sma13` | good | 上一 down 段内 **SMA_13 自身的最低值**（v3.1 修复：独立追踪最小值） |
| `prev_seg_low_way_s_way` | good | SMA13 最低值对应 K 线的 way_s_way |
| `prev_seg_low_vol_way_s_way` | good | SMA13 最低值对应 K 线的 vol_way_s_way |
| `prev_seg_high_price` | bad | 上一 up 段最高价 |
| `prev_seg_high_sma13` | bad | 上一 up 段内 **SMA_13 自身的最高值**（v3.1 修复：独立追踪最大值） |
| `prev_seg_high_way_s_way` | bad | SMA13 最高值对应 K 线的 way_s_way |
| `prev_seg_high_vol_way_s_way` | bad | SMA13 最高值对应 K 线的 vol_way_s_way |
...
**整理时间：** 2026-06-20（v3.1 SMA13 极值追踪修复）
**更新内容：**

- **v3.1**（2026-06-20）：修复止损倒挂。prev_seg_low_sma13 改追踪 SMA13 最低值，prev_seg_high_sma13 改追踪 SMA13 最高值，两者与价格极值独立追踪。倒挂止损从 31 个（21.1%）降为 0。
