# -*- coding: utf-8 -*-
"""v5c: 修复做多侧 way_s_way 符号 bug（down 段 wsw 为正），重测超跌做多"""
import sys
...
def run_long_fixed(symbol, thr, thr_rise, thr_w, stop_pct=1.2, tp_r=3.0, cost=0.05):
    ...
    for i in range(1, n):
        ...
        if pos is None:
            sig = 0
            # down 段 wsw 为正：要求 >= thr_w（下跌结构质量高）
...
for symbol in ["XAUUSDm", "USOILm"]:
    print("=" * 26, symbol, "做多侧（符号修复）", "=" * 26)
    rows = []
    for gt in [2.0, 2.5, 3.0]:
        for tr_ in [1.0, 1.5, 2.0, 2.5]:
            for tw in [0.3, 0.4, 0.5]:
                tr = run_long_fixed(symbol, gt, tr_, tw)
                m = stats(tr)
                if m and m["n"] >= 10:
    ...
    if rows:
        ...
        print("  推荐: gate=%s rise=%s w=%s  n=%d pf=%.3f pnl=%.1f" % (gt, tr_, tw, m["n"], m["pf"], m["pnl"]))
        print(df.groupby("year").agg(n=("pnl_points", "size"), pnl=("pnl_points", "sum")).round(2).to_string())
        print("  exit:", tr["exit_reason"].value_counts().to_dict())
    else:
        print("  （无 n>=10 组合）")
    print()
