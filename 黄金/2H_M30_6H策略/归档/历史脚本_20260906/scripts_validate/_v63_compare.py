...
print("\n交易数: EA", len(ea_tk), " Python", len(py_tk))
print("交易级 missing:", len(py_tk - ea_tk), " extra:", len(ea_tk - py_tk))

# missing/extra 的 mode
miss_tk = py_tk - ea_tk
extra_tk = ea_tk - py_tk
miss_df = py[py['trade_k'].isin(miss_tk)].drop_duplicates('trade_k') if False else py[py['entry_ts'].dt.strftime('%Y.%m.%d %H:%M:%S')+'|'+py['dir'].isin(miss_tk)].drop_duplicates(['entry_ts','dir'])
extra_df = ea[ea['entry_ts'].dt.strftime('%Y.%m.%d %H:%M:%S')+'|'+ea['dir_n'].isin(extra_tk)].drop_duplicates(['entry_ts','dir_n'])
print("\nmissing mode:", miss_df.groupby('mode').size().to_dict())
print("extra mode:", extra_df.groupby('mode').size().to_dict())
