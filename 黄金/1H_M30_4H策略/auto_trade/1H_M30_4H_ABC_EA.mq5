//+------------------------------------------------------------------+
//|                                  1H_M30_4H_ABC_EA.mq5           |
//|                                                         Codex    |
//|        1H_M30_4H A+B+C combined candidate (2026-08-15)           |
//|  Entry : pre_cross / cross / post_n(2-6), previous-segment SMA13 |
//|          extreme stops (post_n: current-bar SMA13), next-bar open|
//|  Gate  : 4H pool (extreme vs SMMA55 >=2%) + H4 bias5 same-side   |
//|          >=0.6% + 1H bias5&13 signed>0; optional G4 short veto   |
//|  Exit  : 3-stage 2.0R / 1.5R trail + 4.0R / M30 merged cross     |
//|  Risk  : 1% balance, stage units 0.5/1.0/1.5, SimMode only       |
//|  IMP   : post_n5/post_n6 removed, Stage1 1.5R (validated)        |
//+------------------------------------------------------------------+
...
input group "=== Account ==="
input ulong   InpMagic      = 312036;      // Magic Number
...
input bool    InpDropPostN5  = true;       // IMP: drop post_n5
input bool    InpDropPostN6  = true;       // IMP: drop post_n6
input int     InpSameDirCdBars = 6;        // post_n 同向冷却（M30 bar，0=关闭）：叉后追势信号最小间隔
input int     InpLossCdLoss   = 2;        // 损失冷却：连续亏损信号组达到该次数后暂停开仓（0=关闭）
input int     InpLossCdHours  = 120;      // 损失冷却时长（小时）
input double  InpPostNMult   = 1.0;        // post_n 手数系数（1.0=正常，0.5=半仓）

input group "=== 1H Gate (4H pool + H4 bias5 + 1H bias5&13) ==="
input double  InpPoolBias55MinPct = 2.0;   // 4H pool: extreme vs H4 SMMA55 >= X%
input double  InpBias5MinPct = 0.6;        // H4 bias5 (extreme vs SMMA5) same-side >= X%
input bool    InpVetoShortH4Uptrend = false; // G4: reject SELL when H4 SMMA5>SMMA13
input int     InpH1SMA5     = 5;           // H1 SMMA5
input int     InpH1SMA13    = 13;          // H1 SMMA13
input int     InpH4SMA5     = 5;           // H4 SMMA5
input int     InpH4SMA55    = 55;          // H4 SMMA55
input int     InpM30SMA5    = 5;           // M30 SMMA5
input int     InpM30SMA13   = 13;          // M30 SMMA13

input group "=== Split TP (three stages) ==="
input double  InpStage1R      = 1.5;       // Stage 1 TP in R (IMP: 1.5R)
...
//--- globals
string g_signals_csv  = "1H_M30_4H_abc_signals_export.csv";
string g_ledger_csv   = "1H_M30_4H_abc_trade_ledger.csv";
int      g_ledger_handle = INVALID_HANDLE;  // BUG-12 (A2 同步): 台账句柄保持打开, 避免频繁 open/close 丢失写入
...
{
   // BUG-13(2026-09-06, A2 同步): 对齐 Python filter_short_segments_causal(逐bar+stack,
   // 吸收短段时只改当前穿越点, 不改 prev/段内). 2H/30m2H 对齐时已实证此版本。
...
//+------------------------------------------------------------------+
//| H1 bias (signed in trade direction), last closed H1 bar            |
//+------------------------------------------------------------------+
double H1BiasPct(int period, int dir_sign)
{
   MqlRates h1[];
   int n = CopyRates(InpSymbol, PERIOD_H1, 0, 60, h1);
   if(n < period + 2) return EMPTY_VALUE;
   double sma[];
   if(!CalcSmmaArray(h1, n, period, sma)) return EMPTY_VALUE;
   int last_closed = n - 2;
   double close = h1[last_closed].close;
...
//+------------------------------------------------------------------+
//| 4H pool + H4 bias5 on the roll4 extreme price (last 4 closed H1)  |
//+------------------------------------------------------------------+
bool Pass1HGate(int dir_sign)
{
   double b5_h1 = H1BiasPct(InpH1SMA5, dir_sign);
   double b13_h1 = H1BiasPct(InpH1SMA13, dir_sign);
   if(!MathIsValidNumber(b5_h1) || !MathIsValidNumber(b13_h1)) return false;
   if(b5_h1 <= 0.0 || b13_h1 <= 0.0) return false;

   MqlRates h1[];
   int n1 = CopyRates(InpSymbol, PERIOD_H1, 0, 60, h1);
   if(n1 < 10) return false;
   int h1_last = n1 - 2;
   double extreme = 0.0;
   for(int i = MathMax(0, h1_last - 3); i <= h1_last; i++)
   {
      if(dir_sign > 0)
         extreme = (extreme == 0.0) ? h1[i].low : MathMin(extreme, h1[i].low);
      else
         extreme = (extreme == 0.0) ? h1[i].high : MathMax(extreme, h1[i].high);
   }

   MqlRates h4[];
   int n4 = CopyRates(InpSymbol, PERIOD_H4, 0, 300, h4);
   if(n4 < InpH4SMA55 + 2) return false;
   double sma5[], sma55[];
   if(!CalcSmmaArray(h4, n4, InpH4SMA5, sma5)) return false;
   if(!CalcSmmaArray(h4, n4, InpH4SMA55, sma55)) return false;
   int h4_last = n4 - 2;
   double s5 = sma5[h4_last];
   double s55 = sma55[h4_last];
   if(s5 == 0.0 || s55 == 0.0 || !MathIsValidNumber(s5) || !MathIsValidNumber(s55)) return false;

   double bias55_ext = (extreme - s55) / s55 * 100.0;
   double bias5_ext = (extreme - s5) / s5 * 100.0;
   if(bias55_ext * dir_sign > -InpPoolBias55MinPct) return false;
   if(bias5_ext * dir_sign > -InpBias5MinPct) return false;

   if(InpVetoShortH4Uptrend && dir_sign < 0 && s5 > s55) return false;
   return true;
}
...
void ProcessOpenTrades(const MqlRates &rates[], int n, const double &sma13[], const int &merged[])
{
   ...
   for(int s = 0; s < ArraySize(g_trades); s++)
   {
      ...
      // BUG-修复 2026-09-01: RESTORED 仓(open_seq==0)由 RestoreRealPositions 在重启后恢复，
      // 其 open_seq=0 无真实开仓 bar 序号，age 回溯(closed_idx-(age-1))会把 idx 指向滞后/错误的历史 bar，
      // 使 M30 merged cross 的动态止盈(及 bound)永远检测不到(例: 2026-08-31 20:30 上穿未触发平仓)。
      // 修复：恢复仓始终检查"最新关闭 bar"(idx=closed_idx)，逐根新 bar 实时判定 bound/SL。
      int idx = closed_idx;  // P0-1 同类修复(2026-09-04): 出场逐 bar 推进, 原 age 倒推冻结在开仓 bar
      ...
      if(!g_trades[s].stage1_done)
      {
         ...
         if(is_long)
         {
            ...
            else if(bar.high >= target) { exit_px = target; reason = "Stage1 TP"; g_trades[s].stage1_pnl = InpStage1R * g_trades[s].r; exit_now = true; }
         }
         else
         {
            if(bar.high >= g_trades[s].stop) { exit_px = g_trades[s].stop; reason = "SL hit"; g_trades[s].stage1_pnl = -g_trades[s].r; exit_now = true; }
            else if(bar.low <= target) { exit_px = target; reason = "Stage1 TP"; g_trades[s].stage1_pnl = InpStage1R * g_trades[s].r; exit_now = true; }
         }
         if(exit_now)
         {
                  ...
                  const int &raw[], const int &merged[], int idx, int &dir, int &mode, double &stop)
{
   ...
   // cross first (BUG-15, A2 同步: Python cross 检测用 raw 方向, 非 merged)
   ...
   {
      if(InpDropPostN5 && MathAbs(pn) == 5) return false;
      if(InpDropPostN6 && MathAbs(pn) == 6) return false;
      bool is_long = pn > 0;
      double sl = sma13[idx];
      if(sl != 0.0)
      {
...
int OnInit()
{
   ...
   Print("1H_M30_4H ABC EA initialized. SimMode=", InpSimMode,
...
void OnTick()
{
   ...
   int completed = n - 1;  // P2-2 (A2, 与30m2H同步): 排除 forming bar, 信号不被未收盘bar回溯改写
   ...
   if(hit && Pass1HGate(dir))
   {
      ...
      if(InpSameDirCdBars > 0 && mode == SIG_POST && dir == g_last_signal_dir &&
         ...
         cd_ok = false;
      if(stop_pts >= spec_lo && stop_pts <= spec_hi && cd_ok)
      {
         g_pending = true;
   ...
   // 3) open pending at current bar open (P2-5 A2: 移到信号检测后同OnTick, 当前bar开盘立即入场, 不晚1bar)
   if(g_pending)
   {
         ...
         if(!InpSimMode)
         {
            ...
            for(int st = 0; st < 3; st++)
            {
               double stage_lot = unit_lot * unit_lots[st]
                                  * ((g_pending_mode == SIG_POST) ? InpPostNMult : 1.0);
               double step = SymbolInfoDouble(InpSymbol, SYMBOL_VOLUME_STEP);
               if(step <= 0.0) step = 0.01;
